...
%smoothing phid
for i=2:length(phid)-1
    phid(i) = (phid(i+1)+phid(i-1))/2;
end
phid = phid';
...
%smoothing thetad
for i=2:length(thetad)-1
    thetad(i) = (thetad(i+1)+thetad(i-1))/2;
end
...
%smoothing thetadd
for i=2:length(thetadd)-1
    thetadd(i) = (thetadd(i+1)+thetadd(i-1))/2;
end
thetadd = thetadd';
